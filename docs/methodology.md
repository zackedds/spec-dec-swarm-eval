...
The harness was originally calibrated at `max_new_tokens=256` (within 0.2% of
1024-token Spec-Bench-leaderboard speedup measurements per Vicuna-era proxy
calibration). In May 2026 we dropped to 128 to halve per-eval wall time for
swarm inner-loop iteration.

The trade-off measured on this exact pair (Llama-3.3-70B + Llama-3.2-1B Q4_K_M):

- Canonical Leviathan baseline at max_new=256: ~2.79× (full tier)
- Canonical Leviathan baseline at max_new=128: ~2.57× (fast tier, observed)

The ~10–12% absolute drop is because spec-dec has per-cycle setup overhead
that's amortized over generated tokens; with fewer generated tokens, the
amortization is worse. The shift is roughly constant across the optimization
landscape, so swarm hill-climbing is unaffected — but **for paper-grade
absolute speedup numbers you should re-run the agent's best recipe at
max_new=256 (or 1024 for leaderboard parity) before publishing.**
